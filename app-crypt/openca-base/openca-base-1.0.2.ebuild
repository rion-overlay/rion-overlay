...
inherit eutils  webapp multilib depend.apache
...
IUSE="dbm mysql +postgres ldap vhosts sasl ca-only ra-only node-only scep full-install db2"

DEPEND="!!app-crypt/openca
	dev-libs/openssl
...
src_configure() {
	...
	use !vhosts && myconf="${myconf} --with-htdocs-url-prefix=/openca"
...
# 	make   DEST_DIR="${D}" install-doc ||die "install failed"
	...
	dodoc CHANGES HISTORY I18N INSTALL README \
		README RELEASE-NOTES STATUS THANKS VERSION
...
pkg_setup() {
	einfo "Creating openca group ..."
	enewgroup openca

	einfo "Creating openca  user"
