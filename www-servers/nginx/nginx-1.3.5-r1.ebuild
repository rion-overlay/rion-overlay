...
# NginX naxsi module ( http://code.google.com/p/naxsi/, GPL-2)
# apache mod_security analog for NginX
HTTP_NAXSI_MODULE_PV="O.49"
HTTP_NAXSI_MODULE_P="naxsi-core-${HTTP_NAXSI_MODULE_PV}"
...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_naxsi? ( http://naxsi.googlecode.com/files/naxsi-core-${HTTP_NAXSI_MODULE_PV}.tgz -> ${HTTP_NAXSI_MODULE_P}.tgz )
...
http_echo http_naxsi http_memc http_drizzle http_rds_json http_postgres http_coolkit
...
# http_naxsi
	if use nginx_modules_http_naxsi; then
		http_enabled=1
		 myconf="${myconf}  --add-module=${WORKDIR}/naxsi-core-0.49/naxsi_src/ "
	fi
...
# http_naxsi initial rules
	if use nginx_modules_http_naxsi; then
		insinto /etc/nginx
		doins "${WORKDIR}/naxsi-core-0.49/"naxsi_config/*
	fi
